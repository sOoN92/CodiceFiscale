...
public class GestioneCf {
	...
	private CodiceFiscale calcoloCodiceFiscale(String cognomecf,String nomecf,String dataNascitacf,String comuneNascitacf,String sesso) {
		...
		for (int i = 0; i < nomecf.length(); i++) {
			if (cont == 3)
				break;
			if (nomecf.charAt(i) != 'A' && nomecf.charAt(i) != 'E' && nomecf.charAt(i) != 'I' && nomecf.charAt(i) != 'O'
					&& nomecf.charAt(i) != 'U') {
		...
		/* comune nascita */
	    comuneNascitacf = comuneNascitacf.toLowerCase();
	   ...
	   if(codReg != null){
		...
		codiceFiscale.setCodiceFiscale(codFis);
		codiceFiscale.setCognome(cognomecf);
		codiceFiscale.setComune(comuneNascitacf);
